...
OUTLIER_THRESHOLD = 490  # Remove rows where Area_Trigger < this value in final output
...
def process_area(file_path, include_extra_points=True, n_keep=N_KEEP, outlier_threshold=OUTLIER_THRESHOLD):
    ...
    for batch_idx, (batch_id, batch) in enumerate(df.groupby('__batch_id')):
        ...
        if len(below_idx) > 0:
            ...
            keep_idxs = list(batch.index[batch.index <= first_below])
            if include_extra_points:
                keep_idxs += list(batch.index[batch.index > first_below][:n_keep])
    ...
    if 'Area_Trigger' in df_out.columns:
        df_out = df_out[df_out['Area_Trigger'] >= outlier_threshold]
...
if __name__ == "__main__":
    file_path = pick_file()
    if file_path:
        process_area(file_path, include_extra_points=True, n_keep=N_KEEP, outlier_threshold=OUTLIER_THRESHOLD)
